...
def split_rand_perm_id_array(array_of_ids, training_perc, validation_perc, rng_seed=0, *args):
    ...
    if num_in_set1 + num_in_set2 > array_len:
        raise ValueError('The sum of percentages exceeds 100%. Please adjust the percentages.')
    ...
    if args:
        ...
        for array in args:
            if array.size(1) != array_len:
                raise ValueError('Array to split does not match length of array of IDs.')
